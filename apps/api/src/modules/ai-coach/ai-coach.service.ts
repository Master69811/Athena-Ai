...
export class AiCoachService {
  ...
  private async buildSystemPrompt(userId: string): Promise<string> {
    ...
    return `You are Athena, the world's most advanced AI personal coach built into the Athena AI fitness platform. You combine the expertise of elite personal trainers, sports scientists, nutritionists, and performance coaches. Always respond in the same language the user uses (Italian or English). Use **bold**, bullet points, and structured markdown for clarity.
...
5. Use markdown formatting: **bold** for emphasis, bullet points for lists, headers for sections
6. If you recommend a change, explain what metric you're optimizing for`;
  }
  ...
  async chat(userId: string, conversationId: string | undefined, message: string) {
    ...
    const history: Content[] = previousMessages
      ...
      .slice(0, -1) // exclude the last user message (we send it via chat.sendMessage)
      .map(m => ({
    ...
    return {
      conversationId: conversation.id,
      message: aiMessage,
      usage: { output_tokens: tokensUsed },
    };
  }

  async *chatStream(userId: string, conversationId: string | undefined, message: string): AsyncGenerator<{ text?: string; conversationId?: string; done?: boolean }> {
    let conversation = conversationId
      ? await this.prisma.aIConversation.findFirst({ where: { id: conversationId, userId } })
      : null;
    ...
    yield { conversationId: conversation.id };
    ...
    const tokensUsed = finalResponse.usageMetadata?.candidatesTokenCount ?? 0;
  ...
  getSuggestedQuestions(profile?: any): string[] {
    const questions = [
      'Come dovrei strutturare la mia settimana di deload?',
      'Cosa dovrei mangiare prima dell\'allenamento oggi?',
      'La mia panca è in stallo — cosa devo fare?',
      'Come posso migliorare la profondità dello squat?',
      'Sto facendo abbastanza volume per i quadricipiti?',
      'Devo allenarmi se ho dormito solo 5 ore?',
      'Come calcolo il mio massimale (1RM)?',
      'Quali sono i migliori esercizi per la catena posteriore?',
    ];

    if (profile?.goalType === 'WEIGHT_LOSS') {
      questions.unshift('A che velocità dovrei perdere peso?', 'Devo fare cardio per perdere grasso più velocemente?');
    } else if (profile?.goalType === 'HYPERTROPHY') {
      questions.unshift('Sto facendo abbastanza serie per la massima crescita muscolare?', 'Di quanta proteina ho davvero bisogno?');
    }
